...
// @desc    count  user
// @route   get api/users/countu
// @access  Private
exports.countuser =asyncHandler(async(req,res,next)=>{
  
  const countutilisateur = await usermodel.find({ role: "user" }).count();
  const countcoatch = await usermodel.find({ role: "formateur" }).count();
  res.status(200).send({utilisateur: countutilisateur, formateur: countcoatch })
});
